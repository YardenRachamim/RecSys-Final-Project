...
train_file_name = 'yoochoose-clicks.dat'
test_file_name = 'yoochoose-test.dat'

dataBefore = data_path / train_file_name  # Path to Original Training Dataset "Clicks" File
dataTestBefore = data_path / test_file_name  # Path to Original Testing Dataset "Clicks" File
dataAfter = data_path  # Path to Processed Dataset Folder
...
######################################################################################################
'''#Separate Data into Train and Test Splits timeMax = data.Time.max() #maximum time in all records sessionMaxTime = 
data.groupby('SessionID').Time.max() #group by sessionID and get the maximum time of each session sessionTrain = 
sessionMaxTime[sessionMaxTime < (timeMax - dayTime)].index #training split is all sessions that ended before the last 
day sessionTest  = sessionMaxTime[sessionMaxTime >= (timeMax - dayTime)].index #testing split is all sessions has 
records in the last day train = data[np.in1d(data.SessionID, sessionTrain)] test = data[np.in1d(data.SessionID, 
sessionTest)] '''
...
# Convert To CSV print('Full Training Set has', len(train), 'Events, ', train.SessionID.nunique(), 'Sessions, and',
# train.ItemID.nunique(), 'Items\n\n') train.to_csv(dataAfter + 'recSys15TrainFull.txt', sep='\t', index=False)
...
trainTR.to_csv(dataAfter / config['train_file_name'], sep=',', index=False)
print('Validation Set has', len(trainVD), 'Events, ', trainVD.SessionID.nunique(), 'Sessions, and',
      trainVD.ItemID.nunique(), 'Items\n\n')
trainVD.to_csv(dataAfter / config['test_file_name'], sep=',', index=False)
