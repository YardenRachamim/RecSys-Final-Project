...
# good checkpoint for yoochoose
# args.load_model = "/content/drive/MyDrive/RecSys/FinalProject/GRU4REC-pytorch-master/checkpoint_yoochoose/02080645/model_00003.pt"
...
parser.add_argument('--batch_size', default=512, type=int) #50 in first paper and 32 in second paper
...
parser.add_argument('--n_epochs', default=20, type=int) #number of epochs (10 in literature)
parser.add_argument('--k_eval', default=20, type=int) #value of K durig Recall and MRR Evaluation
parser.add_argument("--is_bidirectional", action='store_true') #should be used during testing and eliminated during training
...
parser.add_argument('--train_data', default='digenetica_train.txt', type=str)
parser.add_argument('--valid_data', default='digenetica_valid.txt', type=str)
parser.add_argument('--checkpoint_dir', type=str, default='/content/drive/MyDrive/RecSys/FinalProject/GRU4REC-pytorch-master/checkpoint_digenetica/one direction')
parser.add_argument('--results_csv_path', type=str, default='/content/drive/MyDrive/RecSys/FinalProject/documents/hyper-parameters search results/results')
...
def main():
    ...
    loss_functions_ = ["TOP1"] # ["BPR"] 
    
    batch_sizes_ = [100]  # 512
    num_layers_ = [1, 3] # 3
    lr_ = [0.01, 0.03] # 0.03
    hidden_size_ = [1000] # 1000
    n_epochs_ = [10, 20, 5]
    optimizer_ = ["Adagrad"]
    momentum_ = [0]
    dropout_ = [0.5]
                ...
                for optimizer in optimizer_:
                  for momentum in momentum_:
                    for dropout in dropout_:
                        print(f"current parameters: loss_fun: {loss_fun}, num_layers: {num_layers}, hidden_size: {hidden_size}, batch_size: {batch_size}, lr: {lr}, optimizer: {optimizer}, momentum: {momentum}, dropout: {dropout}")
                        ...
                        "batch_size":batch_size, "lr":lr, "n_epochs":n_epochs, "optimizer":optimizer,
